...
resource federatedIdentityCredential 'Microsoft.Graph/applications/federatedIdentityCredentials@v1.0' = if (!empty(managedIdentityClientId) && !empty(managedIdentityPrincipalId)) {
  name: '${appRegistration.uniqueName}/app-function-managed-identity'
  audiences: [
    'api://AzureADTokenExchange'
  ]
  issuer: '${environment().authentication.loginEndpoint}${tenant().tenantId}/v2.0'
  subject: managedIdentityPrincipalId
  description: 'Federated identity credential for Function App managed identity'
}
...
output federatedIdentityCredentialName string = (!empty(managedIdentityClientId) && !empty(managedIdentityPrincipalId)) ? 'app-function-managed-identity' : ''
